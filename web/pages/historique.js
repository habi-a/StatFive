...
const Historique = () => {
  ...
  const getMatch = async (idComplex) => {
    ...
    const userInfo = JSON.parse(user);
    if (userInfo && userInfo.role < 2) {
      const result = await axios.get(API_URL + `/match/get-my-match`, {
      ...
      const resultat = await axios.get(
        API_URL + "/match/get-match-by-complex/" + idComplex,
        { headers: { "api-token": token } }
      );
  ...
  const getAllComplexe = async () => {
    ...
    const userInfo = JSON.parse(user);
    if (userInfo && userInfo.role === 2) {
      const result = await axios.get(API_URL + `/admin/list-complex`, {
        headers: { "api-token": token },
      });
      if (!result?.data.error) {
  ...
  return (
    <Box>
      ...
      >
        ...
        {allComplexe?.length > 0 && (
          <Select
            w="25%"
            m="5px auto"
            placeholder="Choisir un complexe"
            onChange={(e) => e.target.value && getMatch(e.target.value)}
          >
            {allComplexe &&
              allComplexe.length > 0 &&
        ...
        {matchList && matchList.length < 1 && (
          <Heading mt="25px" w="100%" size="md" textAlign="center">
              ...
              return (
                <AccordionItem key={i}>
                  <h2>
                    <AccordionButton
                      onClick={() => {
                        elm.finish && openModal(elm.id);
                      }}
                      _expanded={{ bg: "#002B5B", color: "white" }}
                    >
                      <Box flex="1" textAlign="left">
                        {!elm.finish && "ANALYSE EN COURS - "} Match n° {elm.id}{" "}
                        | Match {elm.name}
                      </Box>
                      <AccordionIcon />
                    </AccordionButton>
                    ...
                    <Flex h="auto">
                      <Box w="50%">
                        <Text
                          ...
                          color="white"
                          textAlign="center"
                          borderRight="1px solid #002B5B"
                          borderBottom="1px solid #002B5B"
                        >
                          {matchData && matchData.team_blue.name}
                        </Text>
                        <Text
                          p="3"
                          fontSize="18px"
                        ...
                        >
                          But : {matchData && matchData.team_blue.goals}
                        </Text>
                        <Text
                          p="3"
                          fontSize="18px"
                        ...
                        >
                          Possession :{" "}
                          {matchData &&
                            Math.round(matchData.team_blue.possesion)}{" "}
                          %
                        </Text>
                      </Box>
                      <Box w="50%">
                        <Text
                          ...
                          color="white"
                          textAlign="center"
                          borderLeft="1px solid #002B5B"
                          borderBottom="1px solid #002B5B"
                        >
                          {matchData && matchData.team_red.name}
                        </Text>
                        <Text
                          p="3"
                          fontSize="18px"
                          ...
                          borderBottom="2px solid #002B5B"
                        >
                          But : {matchData && matchData.team_red.goals}
                        </Text>
                        <Text
                          p="3"
                          fontSize="18px"
                        ...
                        >
                          Possession :{" "}
                          {matchData &&
                            Math.round(matchData.team_red.possesion)}{" "}
                          %
                        </Text>
                      </Box>
                    </Flex>
                    <Text textAlign="center" fontWeight="700" fontSize="26px">
                      Vidéo du match
                    </Text>
